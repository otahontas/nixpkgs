...
if [ "$current_version" = "$latest_version" ]; then
  echo "auth0-deploy-cli already at $current_version"
  exit 0
fi

echo "Updating auth0-deploy-cli from $current_version to $latest_version"
...
(
  ...
  npm install --package-lock-only --ignore-scripts --omit=dev >/tmp/auth0-deploy-cli-install.log 2>&1
)
...
echo "updated auth0-deploy-cli to $latest_version"
