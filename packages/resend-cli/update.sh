...
if [ "$current_version" = "$latest_version" ]; then
  echo "resend-cli already at $current_version"
  exit 0
fi

echo "Updating resend-cli from $current_version to $latest_version"
...
(
  ...
  tar -xzf "$pack_file"
  cd package
  npm install --package-lock-only --ignore-scripts --omit=dev >/tmp/resend-cli-install.log 2>&1
...
echo "updated resend-cli to $latest_version"
