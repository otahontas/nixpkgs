...
if [ "$current_version" = "$latest_version" ]; then
  echo "effect-tsgo already at $current_version"
  exit 0
fi

echo "Updating effect-tsgo from $current_version to $latest_version"

source_nix_hash=$(nix-prefetch-url --type sha256 "https://registry.npmjs.org/@effect/tsgo/-/tsgo-${latest_version}.tgz")
...
(
  ...
  npm install --package-lock-only --ignore-scripts --omit=dev >/tmp/effect-tsgo-install.log 2>&1
)
...
echo "updated effect-tsgo to $latest_version"
